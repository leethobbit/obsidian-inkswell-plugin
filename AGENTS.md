...
8. **Drafts of one story group by `longform.title`, not by folder or basename** — symptom: a feature that wants "all drafts of this story" walks folders or matches index basenames and misses some. Cause: drafts are separate index notes sharing `title`, distinguished by `draftTitle` (Longform's model); grouping lives in `src/projects/stories.ts` (`groupIntoStories`). Each draft is still one `Project`/index path, so project-scoped panels work unchanged. **Consequence for codex:** `codex-project` resolves by the index note's **basename** (`src/codex/codex-scope.ts`), and a new draft has a different basename — so **project-scoped** codex entities bind to the draft they were created under and won't appear in sibling drafts. Series-scoped and global entities carry across (shared series name / global). Recommend series/global scope for multi-draft stories; scoping codex to the story `title` is a future enhancement, not yet done.

...
14. **A project's "name" is three loosely-coupled things** — symptom: a user renames a folder / index note / `longform.title` by hand and the Plan prose goes blank, codex entries vanish, or the wrong project is focused. Cause: the index **path** is the runtime id (`activeProject`, tracker baselines), the index **basename** is the codex `scope.project` key (stored as a `[[wikilink]]`), and `longform.title` drives story grouping + the `<Title> — Plan.md` default; `overview.planningNote`/`cover`/`sceneTemplate` are plain path strings Obsidian never rewrites. Fix: never store a new derived path without a resolver that tolerates it going stale (`findPlanningNote` is the model), and route any rename through `planProjectRename` (`src/projects/rename-plan.ts`, pure + tested) → `executeProjectRename`, which remaps all of the above. `fileManager.renameFile` can await Obsidian's "Update links?" dialog when auto-update is off — that's not a hang.

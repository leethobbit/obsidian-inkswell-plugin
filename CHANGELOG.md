...
### Added
- **Jump from a revision decision to its scene.** In **Revise → Log**, a decision anchored to a scene now shows that scene as a link (and an "Open scene in Write" item in its ⋯ menu) — click to open it straight in the Write editor, so you can act on the decision instead of just reading it.

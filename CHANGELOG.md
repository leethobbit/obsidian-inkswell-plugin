...
- **Revise → Log and Revise → To-dos are now one surface: Revise → To-dos.** Everything left to fix — the inline `[TODO:]`/`[RESEARCH:]` markers in your prose *and* your logged revision decisions — in a single scene-grouped list, the same view the Write sidebar already shows. One chip row filters by marker kind or decision type; tick a decision applied, click it to edit, click a marker to jump to it in Write. Decisions now list highest-priority first within each scene (in the Write sidebar too). On phones, the More → To-dos screen now includes your decision log with full editing (it was markers-only). The "Open revision log" and "Open to-dos" commands both land on the merged view, and hotkeys are untouched.

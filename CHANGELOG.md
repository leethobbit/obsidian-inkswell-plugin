...
- **Project, draft, and scene titles can no longer produce unsafe file names.** A title consisting of dots (`.` / `..`) or with leading/trailing dots could create hidden, invalid-on-Windows, or folder-escaping paths; such titles are now rejected/cleaned before any file is created.

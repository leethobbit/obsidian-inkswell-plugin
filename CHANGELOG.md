...
### Added
- **Jump from a revision decision to its scene.** In **Revise → Log**, clicking a decision that's anchored to a scene now opens that scene straight in the Write editor (its scene tag is a link too, and there's an "Open scene in Write" item in the ⋯ menu) — so you can act on the decision instead of just reading it. Editing a decision moves to the ⋯ menu; project-wide decisions (no scene) still open the editor on click.

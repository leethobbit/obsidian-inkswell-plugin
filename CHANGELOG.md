...
- **Typewriter mode for the Write editor (opt-in).** Settings → Write editor → *Typewriter mode*, or the command *Toggle typewriter mode (Write editor)*: the line you're typing on stays vertically centered, so your eyes never chase the caret down the page. Keyboard movement recenters too; mouse clicks don't. Turning it on or off never rebuilds the editor, so your undo history survives. Requested in [#38](https://github.com/leethobbit/obsidian-inkswell-plugin/issues/38).
- **Manuscript typography for the Write editor (opt-in).** Settings → Write editor → *Manuscript typography* lays your draft out like a typeset page: every paragraph gets a first-line indent except the first paragraph after a heading or a `---` scene break, and headings are centered. Purely visual — nothing in the file changes, and the font still follows Obsidian's Appearance settings. Also from [#38](https://github.com/leethobbit/obsidian-inkswell-plugin/issues/38).
...
### Fixed
- **Revise → Analysis no longer reads scene frontmatter as prose.** Only the first scene's frontmatter was being stripped before analysis, so every later scene's `status`, `synopsis`, `act`, and `chapter` values leaked into Most-used words and Echoes (the "status idea synopsis" phrase), skewed readability, and quoted synopses were scored as dialogue in Scene composition. Reported in [#38](https://github.com/leethobbit/obsidian-inkswell-plugin/issues/38).
- **Style sheet scan results are clickable.** Under Revise → Audit → Style sheet, each flagged word now opens its scene in the Write editor and flashes the word (the same jump To-dos and Search use); the scene heading opens the scene in Write too. Previously the rows looked clickable and did nothing, and their line numbers were counted from the top of the file including frontmatter, so they didn't match the editor. Reported in [#38](https://github.com/leethobbit/obsidian-inkswell-plugin/issues/38).
...
- **Custom beat-sheet templates.** Add your own outlining structures under Settings → Beat sheet templates and they appear in the Beats template dropdown next to the built-in eight. Type one beat per line ("Name", "Name | purpose", or "25% | Name | purpose" to pin where a beat sits in the book). Deleting a template never touches your notes: projects using it show a missing-template notice with every beat note intact until you re-create it or pick another template.

### Fixed
- **Custom codex types (and the new custom beat templates) now show up in Settings immediately.** On recent Obsidian versions the settings tab renders from a snapshot taken when the plugin loads, so a custom type added mid-session had no row — and no way to edit or delete it — until Obsidian restarted. Both sections now refresh the tab after every add, edit, and delete, and render as proper lists with per-row delete buttons on the new settings UI.
...
- **Typing is no longer interrupted — or lost — when a panel refreshes under your caret.** Every editable field across the app (beat notes, overview fields, audit notes, publish metadata and tracker cells, search boxes, the ideas inbox, "Next up", chapter targets, …) now carries a stable identity, and every refresh path restores focus, caret position, and any text you'd typed but not yet committed into the recreated field. Previously only the Codex and Scene Inspector had this protection — elsewhere, a refresh landing mid-keystroke reset the caret and could drop the in-progress text.
- **A synced edit arriving just after your own save is no longer mistaken for your save.** The "this change was mine, refresh softly" bookkeeping now vouches for exactly one refresh per save instead of a 3-second window — an external change landing in that window (sync, another device) now triggers a real refresh instead of being papered over and later overwritten.
- **Plot-grid plotline changes (add, rename, recolor, reorder, delete) now apply against the current list**, closing the last of the stale-state write paths — two plotline tweaks in quick succession can no longer undo each other.
...
### Fixed
- **The status-bar word count updates as you type.** It only refreshed on disk saves, and live keystroke counting had already absorbed the change by then — so the `x / goal` tally sat frozen until a reload. It now listens to the live word-delta stream (which also respects the "what counts toward goals" toggles).
- **Words written into codex profiles now count.** Profile fields (Description, Significance, Backstory, …) live in frontmatter, which the word counter strips — so writing codex content via the Codex panel never counted toward goals, before or after the codex toggle. Codex-tagged notes now count frontmatter prose too; existing tallies are protected by a one-time baseline migration so the change can't produce a phantom word dump.
- **The first words written into a new or never-tracked note no longer vanish.** Creating a note (a scene, the lazily-created planning note) now sets its word baseline at birth, and project files that never had a baseline get one at startup — previously the entire first save was swallowed as the baseline, which is why words written into a fresh planning note before flipping the toggle could never be recovered by it.
- **Home's "Move down" actually moves the scene down.** An off-by-one in the touch/right-click fallback made it a silent no-op ("Move up" was fine).
